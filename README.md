...
- 倚天（Eten）私用區擴充字元，包括帶圈序號、小寫羅馬數字、日文假名、基里爾字母、「行列 40 輸入法」鍵位符等，部分字元增加 Unicode 正式碼位映射。
...
「晰黑體 B5」「致宋體 B5」中的字元寬度與 Windows「細明體」的字元寬度並不完全相同，因此在替換文件內的 Windows「細明體」時仍可能出現部分字元跑位移動的情況。
